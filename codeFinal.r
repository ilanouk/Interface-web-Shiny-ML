  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Modèles",
                 fileInput("fichier_modele", "Choisir le fichier .data"),
                 actionButton("lancer_modele", "Lancer le modèle"),
                 textOutput("resultats_modele"),
                 plotOutput("courbe_roc") 
        )
...
server <- function(input, output, session) {
  ...
  observeEvent(input$lancer_modele, {
    req(input$fichier_modele$datapath)
    print('O')
    ...
    donnees_modele <- read.table(input$fichier_modele$datapath, header = TRUE, sep = ",")
    model_rf <- fonctionRF(donnees_modele, param_interet <- "X1.2")
    
    res <- getPrecision_Recall_FScore(model_rf[[4]])
    
    output$resultats_modele <- renderText({
      paste("Precision:", res[1], "\n",
            "Recall:", res[2], "\n",
            "Fscore:", res[3], "\n")
    })
    
    output$courbe_roc <- renderPlot({
      afficheROC(model_rf[[1]], donnees_modele, param_interet)
    })
  })
}
